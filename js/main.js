...
function renderPictos(tech){
  return Object.entries(PICTO_CFG).map(([key,cfg])=>{
    ...
    const note2=key==='cout'?`<div class="picto-note">ℹ️ Coûts estimés hors taxes (HT), donnés à titre indicatif et variables selon le contexte du projet.</div>`:'';
    return`<div class="picto-card"><div class="picto-label">${cfg.label}</div><div class="picto-icons">${makePictos(key,note,cfg.color)}</div><div class="picto-detail">${detail}</div>${note2}</div>`;
  }).join('');
}
...
function renderPanel(p,layer){
  ...
  const te_key=p['type_espace']||'';const _idKey=p._layer+'_'+p.fid;const _simg=SPACE_IMAGES_BY_ID[_idKey]||SPACE_IMAGES[te_key]||(()=>{const _ill=p['Illustration']||'';const _m=_ill.match(/src="([^"]+)"/);return _m?_m[1]:null;})();const ill=_simg?'<img src="'+_simg+'">':(p['Illustration']||''),src=_simg?{1:_simg}:ill.match(/src="([^"]+)/);
